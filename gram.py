...
def spectrum_project(Ih, I):
    # projects Ih into the space of images with same spectrum as I
    
    # if there is only one octave the shape is different
    if len(I.shape) == 3:       
        I = tf.reshape(I, [1] + list(I.shape))
        
    s = [I.shape[1], I.shape[2]]
    mean =  Lambda(lambda x : (x[:,:,:,0] + x[:,:,:,1] + x[:,:,:,2])/3)   
    ...
    It_r = spec(Ih_gray, I_r, s)
    It_b = spec(Ih_gray, I_b, s)
    It_g = spec(Ih_gray, I_g, s)
...
def spectrum_loss(pyramid_model, pyramid_target, beta):
    diff_layers = []
...
def synthesize_animation(pyramid_model, gram_model, target_grams, pyramid_target, use_spec, beta,
    ...
    if use_spec:
        spec_loss = spectrum_loss(pyramid_model, pyramid_target, beta)
