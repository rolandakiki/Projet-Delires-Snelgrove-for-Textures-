...
def reduce_layer(a=0.4, padding_mode='valid'):
    ...
    def fn(x):
        return K.conv2d(K.conv2d(x, kernel_3d, strides=(2,1), padding=padding_mode),
                K.permute_dimensions(kernel_3d, (1, 0, 2, 3)),
                strides = (1, 2), padding=padding_mode)
...
############ written by R.Akiki##############
def modify_pyramid(num_octaves, padding_mode='valid'):
    ...
    level = reduce_layer(padding_mode='same')(gaussian_pyramid[-1])
    gaussian_pyramid.append(level)
    for _ in range(1, num_octaves):
        level = reduce_layer(padding_mode=padding_mode)(gaussian_pyramid[-1])
        gaussian_pyramid.append(level)
    return Model(inputs = image_input, outputs = gaussian_pyramid[1:], name="pyramid_mod")
...
############ written by R.Akiki##############
def get_pyramid_for_images(pyramid_model,image_sources, source_width = None, source_scale = None):
    target_sub=[]
    ...
    for i, prepped in enumerate(get_images(image_files, source_width=source_width, source_scale=source_scale)):
        ...
        this_sub = pyramid_model.predict(prepped)
        #this_sub.insert(0,tf.convert_to_tensor(prepped))
        print("got the pyramid!")
        print(len(this_sub))
        target_sub.append(this_sub)
    return target_sub
...
#### written by J. Bujalance ########
from tensorflow.spectral import fft2d, ifft2d
def spectrum_project(Ih, I):
    #I =  tf.cast(_I,tf.float32)
    print("in spectrum project")
    s = [I.shape[1], I.shape[2]]
    
    moy =  Lambda(lambda x : (x[:,:,:,0] + x[:,:,:,1] + x[:,:,:,2])/3)   
    resh = Lambda(lambda x,s=s: tf.reshape(x,s))
    
    Ih_gray = moy(Ih)
    Ih_gray = resh(Ih_gray)

    I_r = I[:,:,:,0]
    I_b = I[:,:,:,1]
    I_g = I[:,:,:,2]
    I_r = tf.reshape(I_r,s)
    I_b = tf.reshape(I_b,s)
    I_g = tf.reshape(I_g,s)
    
    It_r = spectrum(Ih_gray, I_r, s)
    It_b = spectrum(Ih_gray, I_b, s)
    It_g = spectrum(Ih_gray, I_g, s)

    con = Lambda(lambda L : tf.stack(L,-1))
    
    #It = tf.concat([tf.expand_dims(t,-1) for t in [It_r, It_b, It_g]], axis = -1)
    It = con([It_r, It_b, It_g])
    It = Lambda(lambda x : tf.expand_dims(x,0))(It)
    return It

def spectrum(Ih, I, s):
    # Ih is a gray image
    # I is a gray image corresponding to one color channel of the target image
    
    Ih = Lambda(lambda x: tf.cast(x, tf.complex64))(Ih)
    I = Lambda(lambda x: tf.cast(x, tf.complex64))(I)
    
    Ih = Lambda(lambda x: fft2d(x))(Ih)
    I = Lambda(lambda x: fft2d(x))(I)
    Iconj = Lambda(lambda x : tf.conj(x))(I)
    
    tmp = Lambda(lambda x,y=Iconj : x*y)(Ih)
    denom = Lambda(lambda y:tf.abs(y))(tmp)
    denom = Lambda(lambda x: tf.cast(x, tf.complex64))(denom)
     
    phaseIh = Lambda(lambda x,y=denom : x/y)(tmp)
    out = Lambda(lambda x,y=I : x*y)(phaseIh)
    out = Lambda(lambda x:ifft2d(x), output_shape = lambda input_shape: s)(out)
    out = Lambda(lambda y:tf.abs(y))(out)

    return out
    

def spectrum_loss(pyramid_model, pyramid_target):
    beta = 1e-10
    diff_layers = []
    for base, target in zip(pyramid_model.outputs, pyramid_target):
        I_tilde = spectrum_project(base, target)
        print(target.shape)
        print(I_tilde.shape)
        diff_layers.append(Lambda(lambda x, target=target : 0.5*beta*K.sum(K.square(target - x[0])),
                                               output_shape = lambda input_shape: [1])([I_tilde]))
...
def internal_novelty_loss(grams, mul=1.0):
    ...
    # gram will be something like (5*8, 64, 64)
    flat = keras.layers.Flatten()(gram)
...
def synthesize_animation(pyramid_model, gram_model, target_grams, pyramid_target, use_spec,
    ...
    if use_spec:
        spec_loss = spectrum_loss(pyramid_model, pyramid_target)
        spec_loss = PrintLayer("Spectrum Loss")(spec_loss)
        total_loss = keras.layers.add([total_loss,spec_loss])	
    print(total_loss)
    import pdb
